...
export default function PrometheusContent() {
  const { data: buildings } = useBuildings();
  const [step, setStep] = useState(1);
  const [intakes, setIntakes] = useState<PrometheusIntake[]>([]);
  const [selectedIntake, setSelectedIntake] = useState<PrometheusIntake | null>(null);
  ...
  async function loadIntakes() {
    setLoading(true);
    try {
      const res = await fetch("/api/prometheus/intake?status=pending");
      if (res.ok) setIntakes(await res.json());
    } catch {}
    setLoading(false);
  }

  function selectIntake(intake: PrometheusIntake) {
  ...
  async function submitIntake() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/prometheus/intake", {
        method: "POST",
  ...
  async function createDraft() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/prometheus/intake/${selectedIntake.id}/draft`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: draftTitle,
  ...
  async function verifyDraft() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/prometheus/draft/${currentDraft.draft.id}/verify`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({}),
      });
  ...
  async function promoteDraft() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/prometheus/draft/${currentDraft.draft.id}/promote`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
      if (res.ok) {
  ...
  async function downloadPdf() {
    ...
    setPdfLoading(true);
    try {
      const res = await fetch(`/api/prometheus/draft/${currentDraft.draft.id}/pdf`);
      if (res.ok) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h1 className="text-xl font-semibold text-text-primary font-display">Prometheus</h1>
        <span className="text-xs text-text-dim">Intake to Work Order Pipeline</span>
      </div>
